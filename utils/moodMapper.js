...
const validateEmotionConsistency = (bestResult, allResults) => {
  ...
  // Special crying detection across all results
...
function checkForCryingAcrossModels(allResults) {
  ...
  // Crying detection criteria (more sensitive)
  const cryingDetected = (
    (avgNeutral > 0.8 && avgSadness > 0.003) || // High neutral with any sadness
    (avgSadness > 0.02) || // Direct sadness detection
    (avgFear > 0.03 && avgSadness > 0.001) // Fear + sadness combination
  );
